...
// ความจุสูงสุดต่อวัน (Bank + Sindy)
// 13:00–22:00 = 10 ชั่วโมง
...
async function loadCalendar() {
  ...
  for (let d = 1; d <= totalDays; d++) {
    ...
    const count = days[dateStr] || 0; // จำนวนคิว Bank+Sindy
    const density = Math.min(count / MAX_SLOTS_PER_DAY, 1);

    const cell = document.createElement('div');
    cell.className = 'calCell';
    cell.innerHTML = `<div class="calNum">${d}</div>`;

    // ===== ไล่เฉดสีตามความหนาแน่น =====
    if (count > 0) {
      const hue = 120 - (density * 120); // เขียว(120) → แดง(0)
      const light = 65 - (density * 15);
      cell.querySelector('.calNum').style.background =
        `linear-gradient(180deg,
          hsl(${hue}, 85%, ${light + 6}%),
          hsl(${hue}, 85%, ${light}%)
        )`;
      cell.querySelector('.calNum').style.color = '#0b0f1a';
    }
