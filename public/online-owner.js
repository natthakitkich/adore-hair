(() => {
  ...
  renderTimeOptions = function () {
    ...
    const available =
      [...timeSelect.options].filter(option => !option.disabled);

    timeSelect.value =
      available.some(option => option.value === previous)
        ? previous
        : available[0]?.value || '';
  };
  ...
  generateEditTimeOptions = async function (date) {
    ...
    try {
      ...
      const available =
        [...editTime.options].filter(option => !option.disabled);
